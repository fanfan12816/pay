...
use app\common\model\{Merchant,MerchantChannel,Channel,ChannelBank,Language};
use app\common\service\MchSystemService;
use hg\apidoc\annotation as Apidoc;

use app\admin\lists\channel\ChannelLists;
use app\admin\validate\channel\ChannelValidate;
use app\common\service\ConfigService;
use think\exception\ValidateException;
use app\common\service\{FileService,ExcelService};
/**
 * @Apidoc\Title("a通道-通道管理")
 * Author: JackMater
 */
...
class ChannelController extends AdminController {
   ...
   * @Apidoc\Desc("列表")
   * @Apidoc\Method("GET")
   * @Apidoc\Url("admin/v1/channel/lists")
  ...
  public function lists() {
    ...
    # 如果请求头中有携带token
    if ($this -> member_id) {
        return returnDataListsAdmin(new ChannelLists(),1);
   ...
   * @Apidoc\Desc("导出")
   * @Apidoc\Method("GET")
   * @Apidoc\Url("admin/v1/channel/inExport")
   * @Apidoc\Query("keyword", type="string", require=true, desc="关键字搜索")
   * @Apidoc\Query("status", type="number", require=true, desc="状态:0-关闭,1开启")
   * @Apidoc\Query("in_status", type="number", require=true, desc="代收状态:0-关闭,1开启")
   * @Apidoc\Query("out_status", type="number", require=true, desc="代付状态:0-关闭,1开启")
   ...
   */
    public function inExport() {
        if ($this -> member_id) {
            $lists = new ChannelLists();
            $list =$lists->lists();
            $head=[
                ["key"=>"id","title"=>"通道编号","txt"=>""],
                ["key"=>"name","title"=>"通道名称","txt"=>""],
        ...
        $rt=ExcelService::excel($head, $list,"ChannelLists",'通道列表'.$lists->pageNo."_".$lists->pageSize."_".$lists->count());
   ...
   * @Apidoc\Desc("开发者")
   * @Apidoc\Method("POST")
   * @Apidoc\Url("admin/v1/channel/add")
   ...
   * @Apidoc\Param("video_url", type="string", desc="视频地址")
   * @Apidoc\Param("instr_url", type="string", desc="介绍地址")
  ...
  public function add() {
    # 如果请求头中有携带token
    if ($this -> member_id) {
        $params = (new ChannelValidate())->post()->goCheck('add');
        $ip=getClientIP();
        $data=[
            "name"=>input("name",""),
            "desc"=>input("desc",""),
   ...
   * @Apidoc\Desc("开发中")
   * @Apidoc\Method("POST")
   * @Apidoc\Url("admin/v1/channel/edit")
   * @Apidoc\Param("id", type="string", desc="通道id")
   * @Apidoc\Param("name", type="string", desc="通道名称")
   * @Apidoc\Param("desc", type="string", desc="描述")
   * @Apidoc\Param("in_ratio", type="string", desc="入金费率")
   * @Apidoc\Param("out_ratio", type="string", desc="出金费率")
  ...
  public function edit() {
    # 如果请求头中有携带token
    if ($this -> member_id) {
        $params = (new ChannelValidate())->post()->goCheck('edit');
        $id=input("id",0);
        $Model=Channel::where(['id' => $id])->findOrEmpty();
        if($Model->isEmpty()){
   ...
   * @Apidoc\Desc("开发中")
   * @Apidoc\Method("POST")
   * @Apidoc\Url("admin/v1/channel/allot")
   * @Apidoc\Param("id", type="string", desc="通道id")
  ...
  public function allot() {
    # 如果请求头中有携带token
    if ($this -> member_id) {
        $params = (new ChannelValidate())->post()->goCheck('allot');
        $id=input("id",0);
        $mch_id=input("mch_id",0);
        $Model=Channel::where(['id' => $id])->findOrEmpty();
        if($Model->isEmpty()){
           return messageReturn(0,'通道不存在');
        }else{
            $mch=Merchant::where(['id' => $mch_id])->findOrEmpty();
            if($mch->isEmpty()){
               return messageReturn(0,'商户不存在');
            }
            $MchChannel=MerchantChannel::where(['mch_id' => $mch_id,"channel_id" => $id])->findOrEmpty();
            if($MchChannel->isEmpty()){
               
                $ip=getClientIP();
                $data=[
                    "mch_id"=>$mch_id,
                    "channel_id"=>$id,
                    "in_ratio"=>$Model->in_ratio,
                    "out_ratio"=>$Model->out_ratio,
                    "min"=>$Model->min,
                    "max"=>$Model->max,
                    "status"=>$Model->status,
                    "in_per"=>$Model->in_per,
                    "out_per"=>$Model->out_per,
                    "source"=>$Model->source,
                    "in_status"=>$Model->in_status,
                    "out_status"=>$Model->out_status,
                    "is_bank"=>0,
                    "remark"=>$Model->remark,
                    "ip"=>$ip,
                    "update_by"=>$this -> member_id
                ];
                $model=MerchantChannel::create($data);
                Language::create([
                    "mch_id"=>$mch_id,
                    "channel_id"=>$id
                ]);
                ...
                return messageReturn(0,'此通道已经分配给该商户了');
            }
        }
   ...
   * @Apidoc\Desc("完成")
   * @Apidoc\Method("POST")
   * @Apidoc\Url("admin/v1/channel/del")
   * @Apidoc\Param("id", type="string", desc="通道id")
   */
  public function del() {
    # 如果请求头中有携带token
    if ($this -> member_id) {
        $params = (new ChannelValidate())->post()->goCheck('del');
        $id=input("id",0);
        $Model=Channel::where(['id' => $id])->findOrEmpty();
        if($Model->isEmpty()){
           return messageReturn(0,'通道不存在');
