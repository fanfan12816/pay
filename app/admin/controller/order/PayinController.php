...
use app\common\model\{Merchant,PayinOrder,MerchantChannel,Channel,ChannelBank};
use app\common\service\MchSystemService;
use hg\apidoc\annotation as Apidoc;

use app\admin\lists\order\PayinLists;
use app\admin\validate\order\PayinValidate;
use think\exception\ValidateException;
use app\common\service\{PayinCallbackService};
use app\common\service\{FileService,ExcelService,ConfigService};
/**
 * @Apidoc\Title("a订单-代收订单管理")
 * Author: JackMater
 */

class PayinController extends AdminController {
  
  /**
   * @Apidoc\Title("列表")
   * @Apidoc\Desc("列表")
   * @Apidoc\Method("GET")
   * @Apidoc\Url("admin/v1/order/payin/lists")
   * @Apidoc\Query("payer_name", type="string", require=true, desc="用户付款名称搜索")
  ...
  public function lists() {
    ...
    # 如果请求头中有携带token
    if ($this -> member_id) {
        return returnDataListsAdmin(new PayinLists(),1);
   ...
   * @Apidoc\Desc("导出")
   * @Apidoc\Method("GET")
   * @Apidoc\Url("admin/v1/order/payin/inExport")
   * @Apidoc\Query("payer_name", type="string", require=true, desc="用户付款名称搜索")
   * @Apidoc\Query("order_sn", type="string", require=true, desc="平台订单号")
   * @Apidoc\Query("mch_sn", type="string", require=true, desc="商户订单号")
   * @Apidoc\Query("channel_id", type="number", require=true, desc="订单所属通道id")
  ...
  public function inExport() {
    ...
    if ($this -> member_id) {
        ...
        $rt=ExcelService::excel($head, $list,"PayinLists",'代收列表'.$lists->pageNo."_".$lists->pageSize."_".$lists->count());
   ...
   * @Apidoc\Desc("完成")
   * @Apidoc\Method("POST")
   * @Apidoc\Url("admin/v1/order/payin/callback")
  ...
  public function callback() {
    # 如果请求头中有携带token
    if ($this -> member_id) {
        $params = (new PayinValidate())->post()->goCheck('callback');
   ...
   * @Apidoc\Desc("完成")
   * @Apidoc\Method("POST")
   * @Apidoc\Url("admin/v1/order/payin/close")
   * @Apidoc\Param("order_sn", type="string", desc="订单平台编号")
   */
  public function close() {
    # 如果请求头中有携带token
    if ($this -> member_id) {
        $params = (new PayinValidate())->post()->goCheck('close');
        $order_sn=$params['order_sn'];
        $status=5;
        $Model=PayinOrder::where(['order_sn' => $order_sn])->findOrEmpty();
        if($Model->isEmpty()){
   ...
   * @Apidoc\Desc("完成")
   * @Apidoc\Method("POST")
   * @Apidoc\Url("admin/v1/order/payin/notifier")
   * @Apidoc\Param("order_sn", type="string", desc="订单平台编号")
   */
  public function notifier() {
    # 如果请求头中有携带token
    if ($this -> member_id) {
        $params = (new PayinValidate())->post()->goCheck('notifier');
        $order_sn=$params['order_sn'];
        $Model=PayinOrder::where(['order_sn' => $order_sn])->findOrEmpty();
        if($Model->isEmpty()){
           return messageReturn(0,'订单不存在');
   ...
   * @Apidoc\Desc("完成")
   * @Apidoc\Method("POST")
   * @Apidoc\Url("admin/v1/order/payin/del")
   * @Apidoc\Param("order_sn", type="string", desc="订单平台编号")
   */
  public function del() {
    # 如果请求头中有携带token
    if ($this -> member_id) {
        $params = (new PayinValidate())->post()->goCheck('del');
        $order_sn=$params['order_sn'];
        $Model=PayinOrder::where(['order_sn' => $order_sn])->findOrEmpty();
        if($Model->isEmpty()){
           return messageReturn(0,'订单不存在');
