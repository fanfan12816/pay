...
class MySqlBackupServices {
  ...
  public function __construct($config = []) {
    ...
    // 初始化备份文件路径
    $this -> config['path'] = app() -> getRootPath() . 'backup/database/';
    ...
    if (!$this -> checkPath($this -> config['path'])) {
      return json(['code' => 0, 'data' => ['code' => 0, 'message' => '文件不可写']]);
    }
  }
  ...
  public function getFile($type = '', $time = 0) {
    // 校验时间格式
    if (!is_numeric($time)) {
      return json(['code' => 0, 'data' => ['code' => 0, 'message' => '时间格式不正确']]);
    }
    switch ($type) {
      case 'time':
      ...
      case 'timeverif':
        ...
        if (count($list) === $last[0]) {
          ...
          return json(['code' => 0, 'data' => ['code' => 0, 'message' => '文件可能损坏，请检查']]);
        }
      case 'pathname':
  ...
  public function downloadFile($time, int $part = 0, bool $isFile = false) {
    ...
    if (file_exists($fileName)) {
      ...
      return json(['code' => 0, 'data' => ['code' => 0, 'message' => '下载失败,文件可能损坏或好像被外星人带走了.....']]);
    }
  }
  ...
  public function optimize($tables) {
    if ($tables) {
      ...
      if (!$list) {
        return json(['code' => 0, 'data' => ['code' => 0, 'message' => '优化错误,请重试']]);
      }
      return $list;
    } else {
      return json(['code' => 0, 'data' => ['code' => 0, 'message' => '请指定要优化的表']]);
    }
  }
  ...
  public function repair(?string $tables = null) {
    if ($tables) {
      ...
      if ($list) {
        ...
        return json(['code' => 0, 'data' => ['code' => 0, 'message' => '修复错误,请重试']]);
      }
    } else {
      return json(['code' => 0, 'data' => ['code' => 0, 'message' => '请指定要修复的表']]);
    }
  }
